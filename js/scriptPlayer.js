...
function main() {
  ...
  gateDataUrl();                                          // Загрузка канала имени и времени
...
async function fetchDataAndUseIt(index) {
  try {
    // Отправляем запрос к API с помощью функции sendRequest
    // const data = await helpers.sendRequest('GET', 'https://jsonplaceholder.typicode.com/users');
    // console.log(data);
    // temp
    // 
    // 
...
function controlSound(optionSound) {
  ...
  switch (optionSound) {
    ...
    case '*':
      if (video.volume === 0) {
        video.volume = powerVolume; 
      } else {
        powerVolume = video.volume;
        video.volume = 0;
      }
    break;
    case '-':
